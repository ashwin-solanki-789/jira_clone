...
impl JiraDatabase {
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        db_state.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Could not found Epic in Database"))?.stories.push(new_id);
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        for story_id in &db_state.epics.get(&epic_id).ok_or_else(|| anyhow!("Could not find epic in database"))?.stories {
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        let mut db_state = self.database.read_db()?;
        let epic = db_state.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Could not find epic in database"))?;

        let story_index = epic.stories.iter().position(|i| i == &story_id).ok_or_else(|| anyhow!("Could not find story in epic stories vector"))?;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        let story = db_state.stories.get_mut(&story_id).ok_or_else(|| anyhow!("Could not find story in database"))?;
        story.status = status;
...
pub mod test_utils{
    ...
    impl MockDB {
        pub fn new() -> Self {
            Self {
                last_written_state: RefCell::new(
                    DBState{
                        last_item_id: 0,
                        epics: HashMap::new(),
                        stories: HashMap::new()
...
mod tests{
    ...
    mod database {
        ...
        fn read_db_should_fail_with_invalid_path() {
            let db = JSONFileDatabase{file_path:"Invalid Path".to_owned()};
        ...
        fn read_db_should_fail_with_invalid_json() {
            ...
            let db = JSONFileDatabase{file_path: tmpfile.path().to_str().expect("failed to convert tmpfile path into str").to_string()};
