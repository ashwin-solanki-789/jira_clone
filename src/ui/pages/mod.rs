...
impl Page for HomePage{
    fn draw_page(&self) -> Result<()> {
        ...
        for id in epics.keys().sorted() {
            let epic = &epics[id];
            let id_col = get_column_string(&id.to_string(), 17);
            let name_col = get_column_string(&epic.name, 34);
            let status_col = get_column_string(&epic.status.to_string(), 20);
...
impl Page for EpicDetail{
    fn draw_page(&self) -> Result<()> {
        ...
        let id = get_column_string(&self.epic_id.to_string(), 16);
        let name = get_column_string(&epic.name, 16);
        let description = get_column_string(&epic.description, 41);
        let status = get_column_string(&epic.status.to_string(), 19);
        println!("{} | {} | {} | {}",id, name, description, status);
        ...
        for id in epic.stories.iter().sorted() {
            ...
            let story_id = get_column_string(&id.to_string(), 16);
            let name = get_column_string(&story.name, 16);
            let description = get_column_string(&story.description, 41);
            let status = get_column_string(&story.status.to_string(), 20);
            println!("{} | {} | {} | {}", story_id, name, description, status);
        }
...
impl Page for StoryDetail{
    fn draw_page(&self) -> Result<()> {
        ...
        let id = get_column_string(&self.story_id.to_string(), 16);
        let name = get_column_string(&story.name, 16);
        let description = get_column_string(&story.description, 41);
        let status = get_column_string(&story.status.to_string(), 21);
        println!("{} | {} | {} | {}",id, name, description, status);
...
mod tests {
    ...
    mod home_page {
        ...
        fn handle_input_should_return_the_correct_action() {
            ...
            let junk_input = "kkjjsefd";
            let junk_input_with_valid_prefix = "qhju8jg";
